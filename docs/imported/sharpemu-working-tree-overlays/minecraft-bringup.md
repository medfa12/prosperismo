...
## 2026-08-02: the remaining performance gap is architectural

The visually corrected atlas does not explain SharpEmu's remaining frame-time
and memory gap. In the measured run above, one 10.625 MiB atlas payload was
published and reused while process private memory still grew from about 8.2 GiB
to 21.8 GiB. A user-observed KytyPS5 run renders the Minecraft menu, including
its animated tile background, at about 30 FPS while the same host runs
SharpEmu's scene at a small fraction of that rate. The FPS ratio is not yet a
controlled benchmark -- resolution, warm pipeline state and instrumentation
must be matched -- but the retained-memory result independently disproves a
single-texture-copy explanation.

The local renderer differential identifies two general design gaps:

- SharpEmu crosses the AGC/backend seam with immutable per-work snapshots.
  `GuestDrawTexture` carries host pixel arrays, `GuestMemoryBuffer` carries
  captured bytes, and each queued draw/dispatch also owns vertex/index data and
  packed scalar state. `VulkanVideoPresenter` bounds only selected array bytes;
  it removes those bytes from `_pendingGuestWorkBytes` when the render thread
  consumes the CPU work, before the Vulkan submission and its staging,
  descriptors and translated resources retire at a fence. The producer queue
  and submission queue therefore have overlapping ownership which the nominal
  256 MiB pending-work budget does not bound.
- Every SharpEmu Vulkan shader is lowered to a private-register
  `while(active) { switch(pc) }` dispatcher. Even an ordinary reducible guest
  CFG executes one dispatch-loop iteration per guest basic block and carries a
  default 100,000-iteration safety valve. The local KytyPS5 recompiler instead
  builds dominators, post-dominators, loops and merge blocks and emits structured
  SPIR-V for reducible control flow, retaining a dispatcher only as a documented
  fallback. Astro already proves that SharpEmu's dispatcher can dominate a
  frame independently of fragment count; Minecraft has not yet been attributed
  shader-by-shader, so this is a live performance mechanism, not a measured
  share of Minecraft's frame.

KytyPS5's resource side is likewise address/page-owned rather than draw-owned:
one texture cache covers sampled, storage, render-target, depth and video-out
roles; a buffer cache tracks CPU/GPU dirty ranges; descriptor sets are recycled;
and command buffers retain resource handles until a timeline tick. SharpEmu has
important partial caches and submission batching, so the finding is not "no
cache" or "one submit per draw." The missing invariant is that a warm frame
should enqueue guest address + generation + view identity, not another copy of
resource contents, and the one cache owner should retire the referenced host
version at the exact GPU fence.

The next falsifiable checkpoint is a warm-frame census, not another Minecraft
texture patch: record per frame the guest bytes copied/detiled, host upload
bytes, new versus reused image/buffer allocations, descriptor allocations,
pipeline creations, dispatcher steps, command-buffer submissions, fence waits,
and bytes retained in both the CPU-work and GPU-retirement domains. The first
implementation milestone is to replace captured read-only buffer payloads with
guest-address/write-generation references resolved by a page-invalidated
render-thread cache. Correct guest writable buffers remain snapshot/writeback
objects until their ordering contract is represented explicitly.
