...
Prosperismo observes successful guest .bk2 opens and, when a Bink decoder is
...
Prosperismo, and there is no C/C++ code of Prosperismo's own involved in decoding
...
(`src/SharpEmu.Libs/Bink/FfmpegBinkFrameSource.cs`). Prosperismo searches
...
toolchain is required to build Prosperismo; publishing just downloads a zip.
...
degrades gracefully: Prosperismo logs one informational line ("Bink2 bridge
