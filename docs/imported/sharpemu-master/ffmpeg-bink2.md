...
SharpEmu's default AvPlayer path opens native FFmpeg libraries in-process via
`FfmpegNativeVideoDecoder` and `FfmpegNativeAudioDecoder`. The Bink host bridge
likewise tries `FfmpegNativeBinkFrameSource` first. This is the path exercised
by Astro's MP4 intro.
...
`src/SharpEmu.Libs/AvPlayer/AvPlayerExports.cs` can search:
