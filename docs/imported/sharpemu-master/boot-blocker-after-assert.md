...
write** in `TryWriteGuardState` (`ctx.TryWriteUInt64`). SharpEmu has a guest write-watch path
...
inside an HLE export - it is spinning in the generated recursive-mutex stub, which SharpEmu does not
...
Located: a livelocking compare-and-swap in a generated stub SharpEmu does not emit, on the guest's
...
SharpEmu writes `0xC0DEC0DECAFEBA00` to `tlsBase + 0x28` in four places
(`CpuDispatcher.cs:396`, `DirectExecutionBackend.cs:5383`, `HleDataSymbols.cs`,
...
hard precondition, not a debug nicety, so on real hardware it *is* populated and **SharpEmu is
