...
pup_decompress.py establishes that the payload is a run of independently
zlib-compressed 512 KB chunks. Those chunks are consecutive pieces of a
filesystem image, so a file larger than 512 KB straddles a chunk boundary and
cannot be recovered by looking at chunks individually. This decompresses them
in order into a sliding buffer and carves across the seams.
