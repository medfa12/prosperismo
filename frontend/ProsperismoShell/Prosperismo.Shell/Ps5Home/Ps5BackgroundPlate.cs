...
/// <c>bg_hub_default.dds</c> from the dump; failing that the basemat's own
/// measured colour with the host's visible missing-dump notice over it. No
...
{
    /// <summary>
    /// Title id whose hub plate to show when no <see cref="TitleArtPath"/> is
    /// set. Null or "default" selects <c>bg_hub_default.dds</c>; the dump also
    /// carries 14 per-app <c>bg_NPXS400*.dds</c> plates.
    /// </summary>
    ...
    /// background; <see cref="TitleIdProperty"/> is only consulted when this is
    /// null or fails to decode.
    /// </summary>
    ...
    /// Sony value used unmodified, not an approximation of the artwork.
    /// </summary>
    ...
    /// <summary>Test seam: forces hub-plate lookups to this dump root.</summary>
    internal string? DumpRootOverride { get; set; }
    ...
    /// managed firmware's transition point after division by 1920x1080.
    /// </summary>
    ...
    /// <summary>Test seams: normalized firmware ripple origin.</summary>
        ...
        bool animate)
    {
        ...
        try
        {
            ...
            if (bitmap is null)
            {
                // The title ships no artwork, or its artwork would not decode:
                // bg_hub_default, which is exactly what that file is for.
                var dumpRoot = DumpRootOverride;
                var path = await Task.Run(() => ShellBackgroundSource.ResolvePath(titleId, dumpRoot));
                if (path is not null)
                {
                    bitmap = await SharedCache.GetOrAdd(path, static p => Task.Run(() => Decode(p)));
                }
            }
        }
        catch (Exception)
        ...
        Bitmap incoming)
    {
        ...
        try
        {
            ...
            // and all twenty firmware-shader draws run off-thread below.
        ...
        var ebootPath = ResolveRippleEbootPath();
        if (ebootPath is null)
        {
            _nativeRippleFailure = "NPXS40087 eboot.bin is unavailable";
            return null;
        }
        ...
        try
        {
            ...
            {
                if (!Ps5NativeRippleCompiler.TryCompile(
                        ebootPath,
                        out var program,
                        out var error))
                {
        ...
        {
            _nativeRippleFailure = $"firmware ripple: {exception.Message}";
            return null;
        }
    }

    private string? ResolveRippleEbootPath()
    {
        var root = DumpRootOverride ?? RnpsShellAssets.LocateDumpRoot();
        ...
        {
            root,
            Path.Combine(root, "filesystems", "system_ex", "app", "NPXS40087", "eboot.bin"),
            Path.Combine(root, "system_ex", "app", "NPXS40087", "eboot.bin"),
        };
        return candidates.FirstOrDefault(path =>
            path.EndsWith("eboot.bin", StringComparison.OrdinalIgnoreCase) && File.Exists(path));
    }
    ...
    private static Bitmap? Decode(string path)
    {
        try
        {
            ...
            // Decoded at 1920 wide: the plate is exactly 2x the design canvas,
            // so a halved decode is still one source texel per canvas pixel, and
            // full 4K would cost four times the memory for nothing at this
            // presentation size.
