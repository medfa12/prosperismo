...
public sealed record ShellFunctionItem
{
    ...
    /// firmware dump makes it available. Null, or an icon the shell only ships
    /// in vector form, leaves the glyph in place.
    /// </summary>
...
public sealed class ShellFunctionRow : TemplatedControl
{
    ...
    {
        // The shell's own pictogram when the dump has one, else our mark. The
    ...
    /// <summary>
    /// The mark drawn inside one tile: the shell's own icon art when the item
    /// names an icon the firmware dump supplies, otherwise the item's text
    /// glyph. Half the shell's icon set is vector-only and never resolves, so
    /// the glyph path is the normal one, not an error path.
    /// </summary>
    private static Control BuildMark(ShellFunctionItem model, double rest)
    {
        if (model.Icon is { } icon && ShellIcons.TryGet(icon) is { } art)
        {
            var image = new Image
            {
                Source = art,
                Height = Math.Max(10, rest * MarkFraction),
                Stretch = Stretch.Uniform,
                HorizontalAlignment = HorizontalAlignment.Center,
                VerticalAlignment = VerticalAlignment.Center,
            };

            // The art is authored at 64-80 px and drawn larger on a focused
            // tile, so it wants a filtered upscale rather than the default.
