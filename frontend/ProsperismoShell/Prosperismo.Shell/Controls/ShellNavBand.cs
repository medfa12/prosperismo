...
public sealed class ShellNavBand : TemplatedControl
{
    ...
    /// <summary>
    /// The spaces this band shows. Sony mode uses the console's exact pair;
    ...
    ///
    /// <para>Profile is null on purpose and not for want of looking. The console
    /// does not draw a pictogram in that slot at all - it mounts an
    /// <c>Avatar</c> with <c>accountId</c>, <c>isMe: true</c> and
    /// <c>size: SYSTEM_ICON_SIZE</c>, i.e. the signed-in user's own picture.
    /// There is no icon id to recover, and no account to draw, so the slot is
    /// left visibly unfinished rather than filled with a person-shaped mark of
    /// our own invention.</para>
    /// </summary>
    ...
    /// asks for, so nothing is resampled and nothing is approximated. Without a
    /// dump the presenter draws its unresolved marker, which is the intended
    /// outcome: a shell that cannot find Sony's art should look like it cannot
    /// find Sony's art.</para>
    /// </summary>
    ...
    private void UpdateVisuals()
    {
        ...
        {
            // The bundle composes two styles: the base is always bold at
            // SizeLarge, and the unselected one adds normal weight at 0.6.
            bool selected = i == SelectedSpaceIndex;
            _spaces[i].Label.FontWeight = selected ? FontWeight.Bold : FontWeight.Normal;
