...
{
    public ShellTile(string title, string? subtitle = null, IImage? icon = null, object? tag = null)
    {
...
/// <summary>
/// A damped-harmonic-oscillator integrator with the shell's parameters.
/// Avalonia has no spring primitive and its transitions are all duration plus
/// easing, so the row owns this and drives it off its own frame tick.
///
/// Integrated with fixed sub-steps rather than the frame delta so that a long
/// frame cannot make the system diverge, and settled explicitly so the row can
/// stop ticking instead of burning a timer forever.
/// </summary>
internal sealed class ShellSpring
{
    /// <summary>Integration sub-step. Small enough to stay stable for every
    /// preset in the bundle, including the 7000/300 snap.</summary>
    private const double SubStep = 0.001;

    /// <summary>Most catch-up a single tick may apply. A stalled UI thread must
    /// make the strand arrive, not teleport through its own animation.</summary>
    ...
    public bool Advance(double seconds)
    {
        ...
        double remaining = Math.Min(seconds, MaxAdvance);
        double mass = Math.Max(1e-6, _config.Mass);

        while (remaining > 0.0)
        {
            double dt = remaining < SubStep ? remaining : SubStep;
            remaining -= dt;

            // Semi-implicit Euler: acceleration from the current displacement,
            // then position from the updated velocity. Cheap and stable at this
            // step size, and it conserves the settle behaviour of the source.
            double acceleration =
                ((-_config.Stiffness * (_value - _target)) - (_config.Damping * _velocity)) / mass;
            _velocity += acceleration * dt;
            double next = _value + (_velocity * dt);

            if (_config.OvershootClamping && Crossed(_value, next, _target))
            {
                _value = _target;
                _velocity = 0.0;
                Settle();
...
public sealed class ShellTileRow : TemplatedControl
{
    ...
    internal void SetSelectedIndex(int index)
    {
        ...
        // The shell's own focus blip, when a firmware dump supplies it.
    ...
    private FuncControlTemplate BuildTemplate()
    {
        return new FuncControlTemplate((_, ns) =>
        {
            ...
            // TitleContainer, HOME m565 / m214. One line, the experienceName and
            // nothing else, ellipsized rather than wrapped, in a strip exactly
            // SCALED_EXP_SIZE - EXPERIENCE_SIZE tall that is bottom aligned with
            // the focused tile. FontSizePS.SizeNormal resolves through a native
            // module and is not in any bundle, so the pixel value here is a
            // placeholder and must not be tuned against our own screenshots.
    ...
    /// carries the focused experience's name and nothing else. The source runs
    /// one strip per experience with a per-item opacity; ours runs one strip and
    /// swaps the text, which is the same picture with eleven fewer text runs.
    /// </summary>
    private void UpdateCaption()
    {
        ...
        if (SelectedItem is { } sel)
        {
            if (_titleText is not null)
            {
                ...
                _titleText.Width = ShellTitleMetrics.NameWidth(
