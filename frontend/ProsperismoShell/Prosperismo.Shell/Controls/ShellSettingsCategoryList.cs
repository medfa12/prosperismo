...
public sealed class ShellSettingsCategoryList : Panel
{
    ...
    [
        ...
        new("id_prosperismo_about", "About Prosperismo", "users_guide_health_and_safety", "ProsperismoAbout"),
    ];
    ...
    private void PushFocusRect()
    {
        ...
        // Claim the arranged row itself.  The old path independently rebuilt
        // this rectangle from list constants, so any real layout adjustment
        // (viewport clipping, scale rounding, or a future native row metric)
        // left the shared ring visibly detached from the item it framed.
        var rowRect = new Rect(row.Bounds.Size);
        ring.Radius = 0;
        ring.LineScale = ShellSettingsMetrics.FocusLineScale;
        ring.Claim(this, ShellFocusRingTimeline.ApplyListItemStyle(rowRect).TransformToAABB(transform));
    }
}
