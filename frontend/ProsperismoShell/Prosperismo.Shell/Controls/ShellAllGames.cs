...
/// font sizes are the shell's existing placeholders because the bundles resolve
/// every size through a native module.
/// </summary>
public sealed class ShellAllGames : TemplatedControl
{
    // ---- Type scale ------------------------------------------------------
    // Every one of these is a placeholder standing in for a symbolic token
    // (FontSizePS.*) whose pixel value is not recoverable from anything we
    // hold. They are lined up with the values the rest of the shell already
    // uses so the ladder stays consistent; do not tune them against a capture.

    /// <summary>FontSizePS.SizeLarge, as the nav band already renders it.</summary>
    private const double SizeLarge = 28.0;

    /// <summary>FontSizePS.SizeNormal, as the strand caption already renders it.</summary>
    private const double SizeNormal = 22.0;

    /// <summary>FontSizePS.SizeXSmall, as the function row already renders it.</summary>
    private const double SizeXSmall = 15.0;

    /// <summary>FontSizePS.Size2XSmall: one rung under SizeXSmall on the
    /// ladder, one above the Size3XSmall the strand sub-label uses.</summary>
    private const double Size2XSmall = 14.0;

    /// <summary>FontSizePS.Size3XSmall, as the strand sub-label renders it.</summary>
    private const double Size3XSmall = 13.0;
