...
public sealed class ShellFocusRingTimeline
{
    ...
    /// <summary>
    /// The <c>image_focus_noise</c> texture both passes sample, resolved from the
    /// user's <c>Sce.PlayStation.PUI_UI3.rco</c>. Null means no firmware resource
    /// container is available; the payload is never copied out of that dump.
    /// </summary>
    ...
    /// time. Sony derives this from <c>UIValues.CurrentUITime - startTime</c>;
...
/// here. The <em>line</em> is the recovered distance-field band with Sony's
...
public sealed class ShellFocusRing : Control
{
    ...
    /// <param name="rect">The target rect.</param>
    public void Claim(object owner, Rect rect)
    {
        _owner = owner;
        MoveTo(rect);
    }
    ...
    private void RenderHighlight(DrawingContext context)
    {
        ...
        // ShellFocusWash. Sony composites it underneath the persistent line.
        ...
        // The band sits (thickness + offset) outside the target, scaled by the
        // in/out margin, so it comes in from larger and leaves larger.
        double scaleByMargin = _timeline.InOutScale;
        double lineScale = Math.Clamp(LineScale, 0.25, 2.0);
        double band = _timeline.BandWidth * lineScale;
        double inflate =
            (ShellFocusRingTimeline.LineThickness + ShellFocusRingTimeline.LineOffset) *
            scaleByMargin * lineScale;

        var body = rect.Inflate(inflate);
        ...
        double radius = _timeline.CurrentRadius + inflate;
        radius = Math.Max(0.0, Math.Min(radius, Math.Min(body.Width, body.Height) / 2.0));
    ...
    private bool RenderNativeHighlight(DrawingContext context)
    {
        ...
        double inflate =
            (ShellFocusRingTimeline.LineThickness + ShellFocusRingTimeline.LineOffset) *
            _timeline.InOutScale * lineScale;
        var body = rect.Inflate(inflate);
        var surface = body.Inflate((band * 0.5) + 1.0);
        if (surface.Width <= 1.0 || surface.Height <= 1.0)
        {
            return false;
        }

        int width = Math.Max(2, (int)Math.Ceiling(surface.Width));
        int height = Math.Max(2, (int)Math.Ceiling(surface.Height));
        ...
        context.DrawImage(bitmap, new Rect(bitmap.Size), surface);
        return true;
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            // Firmware focus is optional; preserve the shader-derived CPU path.
        }
        finally
        {
            _nativeFramePending = false;
