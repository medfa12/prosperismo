...
/// can still use.
///
/// The picture is rendered live from what was recovered out of the firmware, so
/// there is no movie to find any more and the sequence cannot be missing. The
/// sound is a vsh_asset cue and is only ever read from the user's own disk, so
/// <see cref="ResolveAudioPath"/> returning null is the ordinary case for anyone
/// without a dump and simply means the sequence plays silently. Nothing in this
/// class throws, logs, or touches anything but File.Exists.
/// </summary>
public static class BootIntroPolicy
{
    /// <summary>Environment variable naming the boot sound (file or directory).</summary>
    public const string AudioEnvironmentVariable = "SHARPEMU_BOOT_INTRO_AUDIO";
    ...
    /// <summary>
    /// Sound file names, best match first. sfx_initialboot is the cue the console
    /// pairs with the initial boot movie; sfx_coldboot is the every-boot chime and
    /// only stands in because it is the one a plain firmware dump actually carries.
    /// </summary>
    ...
    /// <summary>
    /// The search bases probed after the configured path and the environment
    /// variable: the firmware dump's vsh_asset directory, then the working
    ...
    internal static IReadOnlyList<string?> DefaultSearchDirectories()
    {
        ...
        if (SafeLocateDumpRoot() is { } dumpRoot)
        {
            directories.Add(Path.Combine(dumpRoot, Path.Combine(VshAssetSegments)));
        }

        directories.Add(SafeGetCurrentDirectory());
    ...
    private static string? SafeLocateDumpRoot()
    {
        try
        {
            return RnpsShellAssets.LocateDumpRoot();
        }
        catch (Exception)
        {
            return null;
