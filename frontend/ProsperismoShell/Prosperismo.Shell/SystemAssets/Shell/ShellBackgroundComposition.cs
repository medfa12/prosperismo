...
/// firmware's, holes included (the obsolete entries are kept so the values line
...
/// 0x2x range that the firmware also accepts belongs to custom theme flows
/// rather than to the light modes named here.
/// </summary>
public enum ShellLightRenderMode
{
    /// <summary>Background renders, no light particles. This is the home-screen mode.</summary>
    NoParticle = 65,
...
/// firmware's three behaviours. There is deliberately no ambient fallback:
...
/// timed animation). Numbering is the firmware's; <c>BootAnimation</c> is an
...
/// <summary>
/// How the shell composes its background, expressed as the numbers the firmware
...
/// UI. The wave and the particles are native effects and are approximated here;
/// everything numeric on this class is recovered rather than invented. See
...
public static class ShellBackgroundComposition
{
    ...
    /// </summary>
    // Measured from the genuine reference capture, not ISA-recovered: the
    // darkest content of ps5oracle/shell_ui/live_background/default.mp4 sits
    // at rgb(5,10,22) over 43 sampled frames. The previously assumed
    // rgb(2,4,8) had the right hue (222.4 deg vs 220.0) but too dark a level.
    // Neither value is firmware-recovered, so the measured one wins.
    // See docs/sony-shell/reference-video-grading.md.
    public static readonly Color BasematColor = Color.FromRgb(0x05, 0x0A, 0x16);
    ...
    /// firmware's own switch, and it is the evidence that the home screen runs
    ...
    public static ShellLightRenderMode LightModeFor(ShellGlobalBackgroundState state)
    {
        return state switch
        {
            ...
            ShellGlobalBackgroundState.ParticleSpread => ShellLightRenderMode.Bottom,
    ...
    /// so the ramp runs at the firmware's rate whatever the host's frame rate
