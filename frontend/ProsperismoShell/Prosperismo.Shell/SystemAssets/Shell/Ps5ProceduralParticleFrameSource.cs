...
/// rather than replayed from firmware textures.
///
/// <para>The two existing frame sources both require Sony's
...
/// are procedural in the firmware too (<c>particle_p</c> performs no texture
...
/// see docs/sony-shell/particle-system.md:</para>
/// <list type="bullet">
...
/// of this class rather than firmware behaviour: the field's world extent, the
...
/// (docs/sony-shell/reference-video-grading.md), not decoded, and are marked
...
internal sealed class Ps5ProceduralParticleFrameSource : IPs5NativeParticleFrameSource
{
    ...
    /// <summary>Park-Miller minimal standard, exactly as the firmware uses it.</summary>
    ...
    private void Step(float time)
    {
        for (var i = 0; i < _particles.Length; i++)
        {
            ...
            // step. No drag and no gravity: the firmware has neither.
            var dx = tx - p.Vx;
            var dy = ty - p.Vy;
            var dz = tz - p.Vz;
    ...
    /// fBm of 4D simplex noise. This is the shape the firmware's three sites
    ...
    // Published MIT algorithm (webgl-noise). The firmware carries these exact
    ...
    /// reference capture, not decoded: the firmware draws this in a pass whose
