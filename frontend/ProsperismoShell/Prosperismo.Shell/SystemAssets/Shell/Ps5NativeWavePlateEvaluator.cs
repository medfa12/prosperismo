...
public static class Ps5NativeWavePlateEvaluator
{
    ...
    /// <summary>Firmware phase written to <c>uCenterPos.z</c> for a frame.</summary>
    ...
    {
        if (Ps5NativeWaveRecordSource.TryLoad(recordIndex, out var firmwareRecord) &&
            firmwareRecord is not null)
        {
        ...
        if (recordIndex is not HomeRecordIndex and not HighContrastRecordIndex)
        {
            throw new InvalidOperationException(
                $"Plane2 record {recordIndex} requires the user's NPXS40087 eboot.bin.");
        }
