...
/// position along the ramp it sits at. This is the firmware's own stop layout -
...
/// <summary>
/// One scalar stop: an amount and the position it sits at. The firmware's alpha
...
///
/// This is not a detail. Every ramp in the firmware's background layer - the six
...
/// construction into a table and sampled by index afterwards. The firmware bakes
...
{
    /// <summary>Texel count the firmware bakes a plate ramp at, and the default here.</summary>
    public const int DefaultResolution = 2160;

    /// <summary>Texel count the firmware bakes a wave ramp at.</summary>
    ...
    /// positions are expected to be non-decreasing, as the firmware's tables are.
    /// </summary>
    ...
    /// <summary>
    /// Evaluates a colour ramp at <paramref name="t"/> with the firmware's spline.
    ...
    /// <summary>
    /// The firmware's basis, written out longhand so it is obvious this is not a
    ...
    // short (three to eight stops), so the firmware's own linear scan is both
