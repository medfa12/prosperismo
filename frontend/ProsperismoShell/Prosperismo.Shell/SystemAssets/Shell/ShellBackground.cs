...
/// <summary>
/// The shell's background: Sony's own plate, and nothing invented on top of it.
///
...
/// particle frame is absent unless the recovered firmware pipeline has
...
/// shaders (M2) and the wave mesh (M3). That audit is superseded: Plane2's
/// exact uniform writer and animated-noise plate pass are now translated here,
/// and the mesh paths are dead in 4.03 rather than a prerequisite. The native
/// renderer and part of its particle event/resource model have since been
/// recovered. Its first visible
/// large-particle pass can be composited here from the firmware-derived cache;
/// continuous execution is still being integrated. The missing mesh content is
/// not consumed by the steady background, so there is no version of
/// "improve the old simulation" that ends in fidelity.</para>
///
...
/// the artwork. And the derived blur companion: the firmware maps a
...
public sealed class ShellBackground : Panel
{
    ...
    /// renderer. The recovered cold-boot pass can now play here from a cache
    /// generated from the user's firmware; disabling this property stops and
    /// hides that pass. Other states remain still until their native programs
    /// and resource timelines are executable.</para>
    /// </summary>
    public static readonly StyledProperty<bool> IsMotionEnabledProperty =
        AvaloniaProperty.Register<ShellBackground, bool>(nameof(IsMotionEnabled), defaultValue: true);

    /// <summary>
    /// Uses the recovered Plane2 / <c>wave_bg_p</c> HomeScreen plate whenever
    /// no title is selected. Sony's 4.03 managed state writer and native node
    /// updater prove that HomeScreen preset 4 selects authored record 2.
    /// Title-focused Home still selects the title's own artwork.
    /// </summary>
    public static readonly StyledProperty<bool> UseNativeWavePlateProperty =
        AvaloniaProperty.Register<ShellBackground, bool>(
            nameof(UseNativeWavePlate), defaultValue: true);

    /// <summary>
    /// Routes Plane2 through the firmware's paired high-contrast state. For
    ...
    /// <summary>
    /// Global background state. The firmware drives its whole background layer
    ...
    /// <summary>
    /// True while a real plate from the dump is on screen; false on the flat
    /// basemat-colour fallback.
    /// </summary>
    public static readonly DirectProperty<ShellBackground, bool> IsDumpAvailableProperty =
        AvaloniaProperty.RegisterDirect<ShellBackground, bool>(
            nameof(IsDumpAvailable), static o => o.IsDumpAvailable);
    ...
    public ShellBackground()
    {
        ...
        //    (2,4,8)/255, flat. A measured Sony value used unmodified. It shows
        //    when the dump is absent, and it is what a plate with alpha
        //    composites over. It is not an impression of the artwork and must
        //    never be graded into one.
        _clearPlate = new Border
        {
            Background = new SolidColorBrush(ShellBackgroundComposition.BasematColor),
        };

        // 2. The firmware's full-screen Plane2 pass. Sony's managed
        ...
        _plate.PropertyChanged += (_, e) =>
        {
            ...
            {
                IsDumpAvailable = e.GetNewValue<bool>();
            }
        };

        // 4. Recovered large-particle pass. The cache is generated from the
        // user's firmware by Ps5ParticleDrawProbe; Plus is the Avalonia
        // equivalent of the ISA-host ONE/ONE/ADD blend. It stays absent when no
        // proven frame exists. The recovered native dispatcher route currently
        // enables the one evaluated raw state (3 / ColdBoot); other states stay
        // absent until their own serialized resource records are executable.
                ...
                StringComparison.Ordinal))
        {
            ...
            // continues to own GlobalState and never promotes a cache frame to
            // steady-home behavior.
    ...
    /// <inheritdoc cref="IsDumpAvailableProperty"/>
    public bool IsDumpAvailable
    {
        get => _isDumpAvailable;
        private set => SetAndRaise(IsDumpAvailableProperty, ref _isDumpAvailable, value);
    }

    /// <summary>
    /// Test seam: forces background lookups to this dump root instead of
    /// <see cref="RnpsShellAssets.LocateDumpRoot()"/>.
    /// </summary>
    internal string? DumpRootOverride
    {
        get => _plate.DumpRootOverride;
        set => _plate.DumpRootOverride = value;
    }

    /// <summary>Test seam: the plate this backdrop draws.</summary>
    internal Ps5BackgroundPlate Plate => _plate;

    /// <summary>Test seam: the translated firmware full-screen plate.</summary>
    internal Ps5NativeWavePlate NativeWave => _nativeWave;

    /// <summary>Test seam: the firmware-derived particle frame layer.</summary>
    ...
    /// </summary>
    internal void RefreshAnimationRoute() => _nativeWave.RefreshAnimationRoute();
    ...
    ///
    /// <para><c>CustomImageRipple</c> executes NPXS40087's original firmware
    ...
    // The firmware turns the global state into a light render mode and, for the
