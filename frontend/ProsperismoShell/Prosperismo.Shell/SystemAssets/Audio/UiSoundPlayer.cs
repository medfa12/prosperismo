...
public static class UiSoundPlayer
{
    ...
    private static MusicVoice? _music;
    ...
    public static void SetMusic(MusicClip? clip, float gain)
    {
        ...
        try
        {
            lock (Gate)
            {
                ...
                _music = new MusicVoice(clip.Samples, clip.LoopStartFrame, clip.LoopEndFrame)
                {
                    TargetGain = gain,
                };
    ...
    private static void RunDevice(int generation)
    {
        ...
        try
        {
            ...
            while (true)
            {
                ...
                lock (Gate)
                {
                    if (Voices.Count > 0 || _music is { IsSilent: false })
                    {
                        lastActive = Environment.TickCount64;
                        continue;
                    }
        ...
        CoreAudioOutput.Run(
            ...
            () =>
            {
                lock (Gate)
                {
                    ...
                    if (Voices.Count == 0 && _music is null or { IsSilent: true })
                    {
                        _pumpRunning = false;
                        return true;
                    }
    ...
    private static bool Mix(short[] scratch, int[] accumulator)
    {
        ...
        lock (Gate)
        {
            if (_music is not null && _music.Mix(accumulator))
            {
                sounded = true;
            }
