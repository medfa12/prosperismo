...
/// and the shell was silent even with a firmware dump present — the sound
...
internal static class CoreAudioOutput
{
        ...
        int idleMilliseconds)
    {
        ...
        OutputCallback callback = (_, _, buffer) => queued.Release();
        var handle = GCHandle.Alloc(callback);

        nint queue = nint.Zero;
        try
        {
            if (AudioQueueNewOutput(ref format, callback, nint.Zero, nint.Zero, nint.Zero, 0,
                    out queue) != 0 || queue == nint.Zero)
            {
                return;
            }
            ...
            {
                if (AudioQueueAllocateBuffer(queue, bufferBytes, out buffers[i]) != 0)
                {
                    return;
                }
            ...
            if (AudioQueueStart(queue, nint.Zero) != 0)
            {
                return;
            }
            ...
            while (!shouldStop())
            {
                ...
                // The callback hands back whichever buffer drained; refilling in
                // rotation keeps the queue saturated without tracking identity.
                for (var i = 0; i < bufferCount; i++)
                {
                    if (queued.CurrentCount == 0)
                    {
                        Submit(queue, buffers[i], scratch, fill, ref idleSince);
                        break;
                    }
        ...
        nint queue, nint buffer, short[] scratch, Func<short[], bool> fill, ref long idleSince)
    {
        ...
        AudioQueueEnqueueBuffer(queue, buffer, 0, nint.Zero);
    }
}
