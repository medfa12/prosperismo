...
namespace SharpEmu.GUI.SystemAssets.Audio;

/// <summary>
/// The home-screen ambient bed: bgm_home.at9 from a user-provided firmware
/// dump, looping under the library the way the console loops it under its own
/// home screen.
///
...
/// so the seam is exactly where Sony put it.
///
...
/// </list>
///
/// Everything degrades silently. With no dump the bed never loads, nothing is
/// logged, and every call here is a cheap no-op. The decode happens on a
/// background task, so neither locating nor decoding an eight-megabyte bed ever
/// touches the UI thread or delays startup. The audio is only ever read from
/// the user's own disk and is never redistributed with the emulator.
/// </summary>
public static class ShellAmbientMusic
{
    ...
    /// <summary>True when a dump containing bgm_home.at9 was located.</summary>
    ...
    public static bool IsAvailable(string? dumpRoot = null) =>
        ShellAudio.GetTrackPath(ShellAudioTrack.HomeBgm, dumpRoot) is not null;
    ...
    /// home screen becomes visible, and a no-op when no dump is present.
    /// </summary>
    /// <param name="dumpRoot">Dump root override; defaults to the located dump.</param>
    public static void Start(string? dumpRoot = null)
    {
        ...
        {
            UiSoundPlayer.SetMusic(ready, ComputeGain());
            return;
        }

        _ = Task.Run(() =>
        {
            var path = ShellAudio.GetTrackPath(ShellAudioTrack.HomeBgm, dumpRoot);
            var clip = At9Music.TryDecode(path, ShellAudio.MakeupGain, forLooping: true);
            ...
            if (clip is not null && wanted)
            {
                UiSoundPlayer.SetMusic(clip, ComputeGain());
            }
        });
    }
    ...
    /// re-reads the dump. Used by tests and by a settings change that repoints
    /// the dump root.
    /// </summary>
    ...
    private static void ApplyGain()
    {
        ...
        UiSoundPlayer.MusicGain = started ? ComputeGain() : 0f;
    }
}
