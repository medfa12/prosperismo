...
//
// Usage:
//   SharpEmu.Tools.GpuConformance <path-to-exec-cs.spv>
//   SharpEmu.Tools.GpuConformance --ps5-particle <path-to-particle_c.spv>
...
public static byte[] RunParticleCompute(Ps5NativeParticleComputeRequest request)
    {
        ...
        lock (ParticleComputeGate)
        {
            var tempRoot = Path.Combine(
                Path.GetTempPath(),
                "sharpemu-particle-compute-" + Guid.NewGuid().ToString("N"));
            ...
            var names = new[]
            {
            "SHARPEMU_PS5_PROBE_TIME",
            "SHARPEMU_PS5_SIMULATION_START",
            "SHARPEMU_PS5_PRE_SIMULATION",
            "SHARPEMU_PS5_COMPUTE_RESOURCES",
            "SHARPEMU_PS5_PARTICLE_IDS",
            "SHARPEMU_PS5_INITIAL_PROPERTIES",
            "SHARPEMU_PS5_ZERO_PROPERTIES",
            "SHARPEMU_PS5_SPAWN_END",
            "SHARPEMU_PS5_PROPERTY_OUTPUT",
            "SHARPEMU_PS5_COMPUTE_RESOURCE_SEQUENCE",
            "SHARPEMU_PS5_COMPUTE_RESOURCE_BANKS",
            "SHARPEMU_PS5_INTERLEAVE_SMALL_DRAW_HISTORY",
            "SHARPEMU_PS5_TRANS_PATTERN_FLAG",
        };
            ...
            try
            {
                ...
                Environment.SetEnvironmentVariable(
                    "SHARPEMU_PS5_PRE_SIMULATION", request.PreSimulation ? "1" : null);
                Environment.SetEnvironmentVariable("SHARPEMU_PS5_COMPUTE_RESOURCES", resourcesPath);
                Environment.SetEnvironmentVariable("SHARPEMU_PS5_PARTICLE_IDS", idsPath);
                Environment.SetEnvironmentVariable(
                    "SHARPEMU_PS5_INITIAL_PROPERTIES",
                    request.InitialProperties.IsEmpty ? null : initialPropertiesPath);
            Environment.SetEnvironmentVariable(
                "SHARPEMU_PS5_ZERO_PROPERTIES", request.ZeroProperties ? "1" : null);
                Environment.SetEnvironmentVariable(
                    "SHARPEMU_PS5_SPAWN_END",
                    request.SpawnEnd?.ToString("0.000000", System.Globalization.CultureInfo.InvariantCulture));
                Environment.SetEnvironmentVariable("SHARPEMU_PS5_PROPERTY_OUTPUT", outputPath);
                Environment.SetEnvironmentVariable(
                    "SHARPEMU_PS5_COMPUTE_RESOURCE_SEQUENCE",
...
public static void Run(string[] args)
    {
        ...
        unsafe
        {
            ...
            var probeTimeText = Environment.GetEnvironmentVariable("SHARPEMU_PS5_PROBE_TIME");
            var probeTime = float.TryParse(
                ...
                : 6.5f;
            var simulationStartText = Environment.GetEnvironmentVariable("SHARPEMU_PS5_SIMULATION_START");
            ...
            var forcePreSimulation = string.Equals(
                Environment.GetEnvironmentVariable("SHARPEMU_PS5_PRE_SIMULATION"),
                "1",
            ...
            if (particleMode)
            {
                ...
                // same ping-pong/property storage so Sony's respawn/integration path
                ...
                if (resourceBanks.Length != 0)
                {
                    ...
                    var propertySequenceOutput = Environment.GetEnvironmentVariable(
                        "SHARPEMU_PS5_PROPERTY_SEQUENCE_OUTPUT");
                ...
                var spawnEndText = Environment.GetEnvironmentVariable("SHARPEMU_PS5_SPAWN_END");
                ...
                for (var step = 0; step < followupSteps; step++)
                {
                    if (particleMode)
                    {
                        if (Environment.GetEnvironmentVariable(
                                "SHARPEMU_PS5_INTERLEAVE_SMALL_DRAW_HISTORY") == "1")
                        {
                        ...
                        if (resourceSequence.Length > step + 1)
                        {
                            ...
                            // Every remaining byte is firmware-authored state
            ...
            if (particleMode)
            {
                ...
                readbackPath = Environment.GetEnvironmentVariable("SHARPEMU_PS5_PROPERTY_OUTPUT")
                    ?? readbackPath;
        ...
        var resources = CreateAcceptedBank1Resources();
                ...
                var zeroProperties = string.Equals(
                    Environment.GetEnvironmentVariable("SHARPEMU_PS5_ZERO_PROPERTIES"),
                    "1",
                    StringComparison.Ordinal);
                var idsOverride = Environment.GetEnvironmentVariable("SHARPEMU_PS5_PARTICLE_IDS");
