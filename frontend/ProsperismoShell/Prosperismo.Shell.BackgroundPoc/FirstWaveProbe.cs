...
/// slices come from
/// <c>docs/sony-shell/firstwave-12.40-shader-contracts.json</c> plus the
/// particle programs located by opcode profile.</para>
///
/// <para>This answers one question only: does <see cref="Gen5ShaderTranslator"/>
/// accept Sony's actual instruction streams? Everything downstream depends on
...
internal static class FirstWaveProbe
{
    ...
    [
        ...
        // The draw pair, from docs/sony-shell/particle-draw.md. An earlier
    ...
    /// this reconstructs the layout from Sony's code rather than from a guess
    ...
    internal static int Dump(string eboot, string stageName)
    {
        ...
        foreach (var instruction in decoded.Instructions.Take(dumpCount))
        {
            ...
            Console.WriteLine(
                ...
                $"{(destinations.Length == 0 ? "-" : destinations),-16} <- {operands}");
        }
        ...
        foreach (var instruction in decoded.Instructions)
        {
            ...
            Console.WriteLine(
                $"  pc=0x{instruction.Pc:X4} {instruction.Opcode,-26} " +
                $"{(destinations.Length == 0 ? "-" : destinations),-12} <- {operands}");
        }
    ...
    /// docs/sony-shell/particle-c-executed.md.</para>
    /// </summary>
    internal static int Compile(string eboot)
    {
        ...
        // docs/sony-shell/particle-record-and-resources.md.
        ...
        // RESOURCES_BIN is the 0xF8-byte block Sony's own pattern blob writes,
        ...
        for (var i = 0; i < uploads.Length; i++)
        {
            ...
            // The property buffer starts as the firmware leaves it: the
    ...
    /// <summary>
    /// Rebuilds <c>particleIds1</c> exactly as the firmware allocator does.
    ///
