...
///   <item>
///     <term><c>SHARPEMU_FAST_BOOT=1</c></term>
...
///   </item>
///   <item>
///     <term><c>SHARPEMU_HDR=0|1</c></term>
...
///   </item>
///   <item>
///     <term><c>SHARPEMU_MAX_WIDTH</c>, <c>SHARPEMU_MAX_HEIGHT</c></term>
...
public static class EmulationCostProfile
{
    ...
    private static void EnsureLoaded()
    {
        ...
        lock (Gate)
        {
            ...
            _fastBoot = ReadFlag("SHARPEMU_FAST_BOOT") ?? false;

            // A fast boot turns these down, but an explicit variable always wins:
            // "fast boot, but I still need HDR" has to be expressible.
            _hdrEnabled = ReadFlag("SHARPEMU_HDR") ?? !_fastBoot;
            _maxWidth = ReadSize("SHARPEMU_MAX_WIDTH") ?? (_fastBoot ? FastBootMaxWidth : 0);
            _maxHeight = ReadSize("SHARPEMU_MAX_HEIGHT") ?? (_fastBoot ? FastBootMaxHeight : 0);
